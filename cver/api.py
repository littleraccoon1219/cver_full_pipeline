...
if FastAPI is None:
    ...
    def dashboard(request: Request):
        return templates.TemplateResponse(
            "dashboard.html", {"request": request, "doctor": CVERPipeline("demo").doctor()}
        )
    ...
    def scan_submit(
        request: Request,
        target: str = Form("demo/nginx:lab"),
        target_kind: str = Form("image"),
        mode: str = Form("full-pipeline"),
        profile: str = Form("demo"),
    ):
    ...
    def api_pipeline(payload: dict[str, Any]) -> dict[str, Any]:
        r = CVERPipeline(payload.get("profile", "demo")).run(
            Target(
                payload.get("target", "demo/nginx:lab"),
                payload.get("target_kind", "image"),
                labels={"cver-lab": str(payload.get("lab_label", "true")).lower()},
            ),
            payload.get("mode", "full-pipeline"),
        )
    ...
    def benchmark_page(request: Request):
        return templates.TemplateResponse(
            "benchmark.html", {"request": request, "benchmark": CVERPipeline("benchmark").benchmark()}
        )
