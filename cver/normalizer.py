...
class FindingNormalizer:
    def normalize(
        self, artifacts: list[ScanArtifact], target: Target, scan_id: str, correlation_id: str
        ...
        for a in artifacts:
            if a.source == "mock-composite":
                ...
                evidences.append(
                    Evidence(
                        "evidence-kata-runtimeclass",
                        a.source,
                        "kata",
                        "runtimeclass",
                        a.data,
                        "Kata RuntimeClass detected or demo evidence",
                        0.8,
                        scan_id,
                        target.target_id,
    ...
    def _mock(self, data: dict[str, Any], target: Target, scan_id: str, corr: str) -> list[Finding]:
        ...
        for f in data.get("findings", []):
            out.append(
                Finding(
                    f.get("finding_id") or stable_id("finding", f.get("dedup_key", f.get("title"))),
                    f.get("source", "mock"),
                    f.get("type", "unknown"),
                    f.get("title", ""),
                    f.get("description", ""),
                    ...
                    f.get("package_name", ""),
                    f.get("installed_version", "unknown"),
                    f.get("fixed_version", "unknown"),
                    f.get("evidence_refs", []),
                    {"raw_source": "data/demo/findings_demo.json"},
    ...
    def _trivy(self, data: dict[str, Any], target: Target, scan_id: str, corr: str) -> list[Finding]:
        ...
        for r in data.get("Results", []) or []:
            for v in r.get("Vulnerabilities", []) or []:
                ...
                out.append(
                    Finding(
                        stable_id("finding", cve, pkg, v.get("InstalledVersion")),
                        "trivy",
                        "vulnerability",
                        v.get("Title") or cve,
                        v.get("Description", ""),
                        ...
                        cve,
                        pkg or r.get("Target", "unknown"),
                        pkg,
                        v.get("InstalledVersion", "unknown"),
                        v.get("FixedVersion", "unknown"),
                        [],
                        {"scanner": "trivy"},
                        0.85,
                        f"{cve}:{pkg}",
        ...
        if hc.get("Privileged") or "SYS_ADMIN" in caps or "CAP_SYS_ADMIN" in caps:
            out.append(
                Evidence(
                    ...
                    {"privileged": hc.get("Privileged"), "cap_add": caps},
                    "Privileged or CAP_SYS_ADMIN detected",
                    0.9,
                    scan_id,
                    target.target_id,
                    correlation_id=corr,
    ...
    def _docker_findings(self, data: dict[str, Any], target: Target, scan_id: str, corr: str) -> list[Finding]:
        ...
        mounts = data.get("Mounts", []) or []
        if any(
            m.get("Destination") == "/var/run/docker.sock" or m.get("Source") == "/var/run/docker.sock" for m in mounts
        ):
            out.append(
                Finding(
                    "finding-docker-sock",
                    ...
                    "Docker socket is exposed to container.",
                    "CRITICAL",
                    "CONFIG-DOCKER-SOCK",
                    "docker",
                    "docker-runtime",
        ...
        if hc.get("Privileged") or "SYS_ADMIN" in caps or "CAP_SYS_ADMIN" in caps:
            out.append(
                Finding(
                    "finding-cap-sys-admin",
                    "docker-inspect",
                    "misconfiguration",
                    "Dangerous capability CAP_SYS_ADMIN",
                    "Container is privileged or has CAP_SYS_ADMIN.",
                    "HIGH",
                    "CONFIG-CAP-SYS-ADMIN",
                    "docker",
                    "container-config",
                    "unknown",
                    "not_applicable",
    ...
    def _k8s_evidence(self, data: dict[str, Any], target: Target, scan_id: str, corr: str) -> list[Evidence]:
        ...
        if spec:
            out.append(
                Evidence(
                    "evidence-k8s-security-context",
                    "k8s-inspect",
                    "k8s",
                    "pod_spec",
                    spec,
                    "Kubernetes Pod spec security context",
                    0.85,
                    scan_id,
                    target.target_id,
                    correlation_id=corr,
    ...
    def _generic_evidence(self, target: Target, scan_id: str, corr: str) -> list[Evidence]:
        return [
            Evidence(
                "evidence-runtime-runc-version",
                "runtime-inspect",
                "runtime",
                "runc.version",
                "1.1.10",
                "runc version 1.1.10 from demo evidence",
                0.85,
                scan_id,
                target.target_id,
                correlation_id=corr,
            ...
            Evidence(
                "evidence-host-kernel",
                "host-inspect",
                "host",
                "kernel",
                "demo-kernel",
                "Host kernel demo evidence",
                0.6,
                scan_id,
                target.target_id,
                correlation_id=corr,
