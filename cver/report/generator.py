...
class ReportGenerator:
    ...
    def markdown(self, p: dict) -> str:
        lines=[f"# CVER Report: {p['target']['name']}","",f"- Scan ID: `{p['scan']['scan_id']}`",f"- Defense Score: **{p['defense_score']['total_score']} / 100**","","## Findings"]
        for f in p["findings"]:
            lines += [f"### {f['severity']} - {f['title']}", f"- Type: `{f.get('macro_type')}/{f.get('fine_type')}`", f"- Root Cause: `{f.get('root_cause')}`", f"- Evidence: `{', '.join(f.get('evidence_refs', []))}`"]
        lines += ["","## Exploitability"]
        for r in p["exploitability_results"]:
            lines.append(f"- `{r['finding_id']}` -> **{r['exploitability_label']}** score={r['exploitability_score']} confidence={r['confidence']}")
            if r.get("reasoning_trace"): lines.append("  - Trace: " + "; ".join(r["reasoning_trace"][:3]))
