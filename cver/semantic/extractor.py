...
class SemanticExtractor:
    def extract(self, finding: Finding, knowledge: CVEKnowledge | None) -> Finding:
        if knowledge:
            ...
            finding.confidence = max(float(finding.confidence), float(sem.get("confidence", 0.5)))
            return finding
        ...
        if finding.type == "misconfiguration":
            finding.macro_type = "misconfiguration"
            finding.root_cause = "misconfiguration_privilege"
        elif "kubernetes" in text.lower() or "k8s" in text.lower():
            finding.macro_type = "orchestration"
            finding.root_cause = "orchestration_config"
        elif any(x in text.lower() for x in ["runc", "containerd", "docker"]):
            finding.macro_type = "runtime"
            finding.root_cause = "runtime_isolation"
        return finding

    def preconditions(
        self, knowledge: CVEKnowledge | None, finding: Finding
    ) -> tuple[list[Precondition], list[Precondition]]:
        if knowledge:
            req = [
                Precondition(**{k: v for k, v in x.items() if k in Precondition.__dataclass_fields__})
                for x in knowledge.semantic_annotations.get("required_conditions", [])
            ]
            blk = [
                Precondition(**{k: v for k, v in x.items() if k in Precondition.__dataclass_fields__})
                for x in knowledge.semantic_annotations.get("blocking_conditions", [])
            ]
            return req, blk
        if finding.fine_type in ("dangerous_capability", "privileged_container"):
            return [
                Precondition(
                    ...
                    "CAP_SYS_ADMIN in container.capabilities OR privileged == true",
                    "rule_inferred",
                    finding.description,
                    0.7,
                    False,
                ...
                Precondition(
                    "drop_cap_sys_admin",
                    "CAP_SYS_ADMIN not in capabilities AND privileged == false",
                    "rule_inferred",
                    "",
                    0.7,
                    False,
