...
class RedTeamExecutor:
    ...
    def execute(
        self, target: Target, scan_id: str, planned: list[dict], execution_level: str, corr: str
    ) -> RedTeamCampaign:
        cid = new_id("campaign")
        results = []
        decisions = []
        for p in planned:
            ...
            action = {
                ...
                "forbidden_actions": s.get("forbidden_actions", []),
            }
            ...
            if not d["allowed"]:
                ...
                results.append(
                    {
                        "scenario_id": s["scenario_id"],
                        "status": "dry_run_triggered" if execution_level == "dry-run" else "safe_exec_simulated",
                        "blocked": False,
                        "defense_observed": False,
                        "finding_id": p.get("finding_id"),
                        "expected_defense_points": s.get("expected_defense_points", []),
                        "retest_logic": s.get("retest_logic"),
                        "poc_policy": "no_real_poc",
                    }
                )
