...
class MockLLMProvider:
    ...
    def complete_json(self, system: str, prompt: str, schema_hint: dict | None = None) -> LLMResponse:
        obj = {
            "mode": "mock",
            "summary": "Mock LLM response; rules remain authoritative.",
            "confidence": 0.5,
...
class OpenAICompatibleProvider(MockLLMProvider):
    ...
    def complete_json(self, system: str, prompt: str, schema_hint: dict | None = None) -> LLMResponse:
        ...
        try:
            ...
            return LLMResponse(
                json.dumps({"error": str(e), "fallback": "mock"}), {"error": str(e), "fallback": "mock"}, self.name, 0.0
            )
