...
def init_db(db_path: str | Path) -> None:
    with conn(db_path) as c:
        ...
        try:
            c.execute(
                "CREATE VIRTUAL TABLE IF NOT EXISTS cve_knowledge_fts USING fts5(cve_id, component, description, semantic_json)"
            )
        ...
        if table == "targets":
            ...
            c.execute(
                ...
                (
                    key,
                    obj.get("profile"),
                    json.dumps(obj.get("metrics", {}), ensure_ascii=False),
                    obj.get("output_path"),
                    obj.get("created_at"),
                    raw,
                ),
...
def audit(db_path: str | Path, record: dict[str, Any]) -> None:
    with conn(db_path) as c:
        c.execute(
            "INSERT INTO audit_log(ts,scan_id,target_id,campaign_id,action,decision,reason,json) VALUES(?,?,?,?,?,?,?,?)",
            (
                now_iso(),
                record.get("scan_id", ""),
                record.get("target_id", ""),
                record.get("campaign_id", ""),
                json.dumps(record.get("action", {}), ensure_ascii=False, default=str),
                record.get("decision", ""),
                record.get("reason", ""),
