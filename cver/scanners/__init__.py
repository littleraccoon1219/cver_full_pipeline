...
class MockCompositeScanner(Scanner):
    ...
    def scan(self, target: Target, profile: dict[str,Any]) -> ScanArtifact:
        ...
        data["target"] = {"target_id": target.target_id, "name": target.name, "kind": target.kind, "labels": target.labels}
...
class TrivyScanner(Scanner):
    ...
    def scan(self, target: Target, profile: dict[str,Any]) -> ScanArtifact:
        if profile.get("scanner", {}).get("use_demo_data") or not self.available():
            return ScanArtifact(self.name, "mock" if profile.get("scanner", {}).get("use_demo_data") else "dry-run", "ok", read_json("data/demo/trivy_image_demo.json"), [])
        ...
        try:
            ...
            return ScanArtifact(self.name, "real-cli", "ok", json.loads(p.stdout or "{}"), [p.stderr] if p.stderr else [])
...
class SyftScanner(Scanner):
    ...
    def scan(self, target: Target, profile: dict[str,Any]) -> ScanArtifact:
        if profile.get("scanner", {}).get("use_demo_data") or not self.available():
            return ScanArtifact(self.name, "mock" if profile.get("scanner", {}).get("use_demo_data") else "dry-run", "ok", read_json("data/demo/syft_sbom_demo.json"), [])
        try:
            p = subprocess.run(["syft", target.name, "-o", "json"], capture_output=True, text=True, timeout=120, check=False)
            if p.returncode != 0:
                return ScanArtifact(self.name, "real-cli", "error", {}, [p.stderr[-1000:]])
...
class DockerInspectScanner(Scanner):
    ...
    def scan(self, target: Target, profile: dict[str,Any]) -> ScanArtifact:
        if profile.get("scanner", {}).get("use_demo_data") or not self.available():
            return ScanArtifact(self.name, "mock" if profile.get("scanner", {}).get("use_demo_data") else "dry-run", "ok", read_json("data/demo/docker_inspect_demo.json"), [])
        if target.kind not in ("image", "container"):
            return ScanArtifact(self.name, "dry-run", "skipped", {}, [])
        try:
            p = subprocess.run(["docker", "inspect", target.name], capture_output=True, text=True, timeout=30, check=False)
            if p.returncode != 0:
                return ScanArtifact(self.name, "real-cli", "error", {}, [p.stderr[-1000:]])
...
class K8sInspectScanner(Scanner):
    ...
    def scan(self, target: Target, profile: dict[str,Any]) -> ScanArtifact:
        if profile.get("scanner", {}).get("use_demo_data") or not self.available() or target.kind not in ("pod","deployment","namespace"):
            return ScanArtifact(self.name, "mock" if profile.get("scanner", {}).get("use_demo_data") else "dry-run", "ok", read_json("data/demo/k8s_inspect_demo.json"), [])
...
class KataInspectScanner(Scanner):
    ...
    def scan(self, target: Target, profile: dict[str,Any]) -> ScanArtifact:
        if profile.get("scanner", {}).get("use_demo_data") or not self.available():
            return ScanArtifact(self.name, "mock" if profile.get("scanner", {}).get("use_demo_data") else "dry-run", "ok", read_json("data/demo/kata_runtimeclass_demo.json"), [])
        try:
            p = subprocess.run(["kubectl","get","runtimeclass","kata","-o","json"], capture_output=True, text=True, timeout=20, check=False)
            if p.returncode != 0:
...
class ScannerManager:
    def __init__(self) -> None:
        ...
        self.scanners = [TrivyScanner(), SyftScanner(), DockerInspectScanner(), K8sInspectScanner(), KataInspectScanner()]
