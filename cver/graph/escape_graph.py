...
class EscapeGraphBuilder:
    def build(
        self, findings: list[Finding], results: list[ExploitabilityResult], scan_id: str, target_id: str, corr: str
    ) -> EscapeGraph:
        nodes = [
            GraphNode(
                ...
                {"boundary": "shared_kernel_or_microvm"},
            )
        ]
        edges = []
        by = {r.finding_id: r for r in results}
        for f in findings:
            ...
            nodes.append(
                GraphNode(
                    ...
                    "Finding",
                    f.title,
                    {"severity": f.severity, "macro_type": f.macro_type, "fine_type": f.fine_type},
                )
            )
            if f.cve_id:
            ...
            for ep in f.escape_primitives or []:
                ...
                nodes.append(GraphNode(pn, "Capability", ep, {}))
                edges.append(GraphEdge(fn, pn, "requires", {}))
                if "host" in (f.impact_boundary or ""):
                    edges.append(
                        GraphEdge(
                            pn, "node-runtime-boundary", "crosses_boundary", {"impact_boundary": f.impact_boundary}
                        )
                    )
            ...
            if r:
                ...
                nodes.append(
                    GraphNode(rn, "ExploitabilityResult", r.exploitability_label, {"score": r.exploitability_score})
                )
                ...
                for m in r.matched_preconditions:
                    ...
                    nodes.append(
                        GraphNode(
                            pn, "Precondition", m.get("name", "precondition"), {"matched": True, "expr": m.get("expr")}
                        )
                    )
                    edges.append(GraphEdge(pn, fn, "matches", {}))
        return EscapeGraph(
            stable_id("graph", scan_id, target_id), nodes, edges, self.mermaid(nodes, edges), scan_id, target_id, corr
        )
